...
STATUS fromGetTokenSignalingState(UINT64 customData, PUINT64 pState)
{
    ...
    if ((SERVICE_CALL_RESULT) ATOMIC_LOAD(&pSignalingClient->result) == SERVICE_CALL_RESULT_OK) {
        ...
        if (ATOMIC_LOAD_BOOL(&pSignalingClient->deleting)) {
            ...
            if (pSignalingClient->pChannelInfo->channelEndpointHttps != NULL && pSignalingClient->pChannelInfo->channelEndpointHttps[0] != '\0' &&
                pSignalingClient->pChannelInfo->channelEndpointWss != NULL && pSignalingClient->pChannelInfo->channelEndpointWss[0] != '\0') {

                STRNCPY(pSignalingClient->channelEndpointHttps, pSignalingClient->pChannelInfo->channelEndpointHttps,
                        MAX_SIGNALING_ENDPOINT_URI_LEN);
                pSignalingClient->channelEndpointHttps[MAX_SIGNALING_ENDPOINT_URI_LEN] = '\0';
                STRNCPY(pSignalingClient->channelEndpointWss, pSignalingClient->pChannelInfo->channelEndpointWss,
                        MAX_SIGNALING_ENDPOINT_URI_LEN);

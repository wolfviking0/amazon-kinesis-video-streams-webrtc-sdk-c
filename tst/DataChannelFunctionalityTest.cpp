...
// Create two PeerConnections and ensure DataChannels that were declared
// before signaling go to connected
TEST_F(DataChannelFunctionalityTest, createDataChannel_partialReliability)
{
    RtcConfiguration configuration;
    PRtcPeerConnection offerPc = NULL, answerPc = NULL;
    PRtcDataChannel pOfferDataChannel = nullptr, pAnswerDataChannel = nullptr;
    SIZE_T datachannelLocalOpenCount = 0, msgCount = 0;
    RtcDataChannelInit partialReliability;
    RemoteOpen remoteOpen{};
    ...
    partialReliability.ordered = TRUE;
    partialReliability.maxPacketLifeTime = 4987;
    partialReliability.maxRetransmits = 0;
    partialReliability.negotiated = FALSE;

    // Create two DataChannels
    EXPECT_EQ(createDataChannel(offerPc, (PCHAR) "Offer PeerConnection", &partialReliability, &pOfferDataChannel), STATUS_SUCCESS);
    EXPECT_EQ(createDataChannel(answerPc, (PCHAR) "Answer PeerConnection", &partialReliability, &pAnswerDataChannel), STATUS_SUCCESS);
